...
RawDataSource::RawDataSource(int n)
  :nBaselines(n)
{}

int RawDataSource::numOfBaselines()const
{
  return nBaselines;
}

bool RawDataSource::fetchOne()
{
  return doFetchOne();
}

std::pair<int,int> RawDataSource::antennaPair(int bl)const
{
  return doAntennaPair(bl);
}

Array<Complex> RawDataSource::data(int field,int band,int bl)const
{
  return doData(field,band,bl);
}

Array<Bool> RawDataSource::flags(int field,int band,int bl)const
{
  return doFlags(field,band,bl);
}

double RawDataSource::time()const
{
  return doTime();
}
  ...
  itsMSCol         (0)
{
  ...
  // Create the MS.
  createMS (msName, antTab, flagColumn, nflagBits);
...
int MSCreate::addBand (int nchannels,
		       double refFreq, double chanWidth)
{
  ...
  return addBand (nchannels, refFreq, chanFreqs, chanWidths);
}
		       ...
		       const double* chanWidths)
{
  ...
  return addBand (nchannels, refFreq, freqs, widths);
}
...
void MSCreate::fillAntenna (const Table& antTab)
{
  ROArrayColumn<double> posCol(antTab, "POSITION");
  ROArrayColumn<double> offsetCol(antTab,"OFFSET");
  Array<double> itsAntPos(posCol.getColumn());
  Matrix<double> itsOffset(Array<double>(offsetCol.getColumn()));
  itsNrAnt=itsAntPos.shape()[1];
  Matrix<double> antPos(itsAntPos);
  ...
  itsArrayPos=new MPosition(antMPos[itsNrAnt/2]);
  itsFrame = new MeasFrame(*itsArrayPos);

  ROScalarColumn<String> nameCol(antTab,"NAME");
  Vector<String> itsAntName(nameCol.getColumn());

  ROScalarColumn<String> stationCol(antTab,"STATION");
  Vector<String> itsStationName(stationCol.getColumn());

  ROScalarColumn<String> mountCol(antTab,"MOUNT");
  Vector<String> itsMount(mountCol.getColumn());

  ROScalarColumn<double> diameterCol(antTab,"DISH_DIAMETER");
  Vector<double> itsDiameter(diameterCol.getColumn());
  ...
  for (Int i=0; i<itsNrAnt; i++) {
    ...
    msantCol.name().put (i, itsAntName[i]);
    msantCol.station().put (i, itsStationName[i]);
    msantCol.type().put (i, "GROUND-BASED");
    msantCol.mount().put (i, itsMount[i]);
    msantCol.positionMeas().put (i, antMPos[i]);
    antOffset[0]=itsOffset(0,i);
    antOffset[1]=itsOffset(1,i);
    antOffset[2]=itsOffset(2,i);
    msantCol.offset().put (i, antOffset);
    msantCol.dishDiameter().put (i, itsDiameter[i]);
...
void MSCreate::fillFeed()
{
  ...
  for (Int i=0; i<itsNrAnt; i++) {
    ...
    msfeedCol.time().put (i, itsStartTime );
    //msfeedCol.interval().put (i, itsNrTimes*itsTimeStep);
    msfeedCol.interval().put (i, 0.);
...
void MSCreate::updateTimes()
{
  ...
  Double interval=itsEndTime-itsStartTime;
  //Double endTime = itsStartTime + interval;
  Double midTime = (itsStartTime + itsEndTime) / 2;
...
void MSCreate::writeTimeStep(RawDataSource& rds)//t in UTC in sec
{
  double t=rds.time();
  assert(t>itsEndTime);
  ...
  for (int field=0; field<itsNrField; ++field)
      ...
      for (int band=0; band<itsNrBand; ++band)
	{
	  ...
	  IPosition shape(2, (*itsNrPol)[band], (*itsNrChan)[band]);
	  ...
	  //std::cerr<<rowNumber<<" "<<itsMS->nrow()<<std::endl;
	  ...
	  for(int bl=0;bl<rds.numOfBaselines();++bl)
	    {
	      std::pair<int,int> antennaPair(rds.antennaPair(bl));
	      int j=antennaPair.first;
	      int i=antennaPair.second;
	      ...
	      itsMSCol->interval().put (rowNumber, dt);
	      itsMSCol->exposure().put (rowNumber, dt);
